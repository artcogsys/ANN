...
class DRQN(object):
    ...
    Data is stored in episodes (could be one huge episode)
    ...
    def __init__(self, ninput, noutput, **kwargs):
        ...
        # size of the replay buffer D (maximal number of episodes)
        self.nepisodes = kwargs.get('nepisodes',10**3)

        # maximal number of encoded time steps per episode
        ...
        # define model
        self.nhidden = kwargs.get('nhidden',1)
        ...
        # initialize replay memory (list of episodes)
        self.D = deque(maxlen=self.nepisodes)

        # do this via truncation?
        # modeling the learning dynamics; making this natural as well
    ...
    def createBuffer(self,n,m):
        ...
        B = {
            ...
            'reward': np.zeros([n, m, 1], dtype=np.float32),
            'obs2'  : np.zeros([n, m, self.ninput], dtype=np.float32),
            'done'  : np.zeros([n, m, 1], dtype=np.bool)
        }
    ...
    def experienceReplay(self):
        ...
        for i in xrange(self.nframes):
            ...
            # learning phase: non-truncated backpropagation
