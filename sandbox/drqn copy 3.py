...
class DRQN(object):
    ...
    def experienceReplay(self):
        ...
        # take steps to collect observations
        for i in xrange(eidx):
            self.model(Variable(B['obs'][:, i]))
            self.target_model(Variable(B['obs'][:, i]))

        # Compute Q1 based on final observation
        Q1 = self.model(Variable(B['obs'][:, eidx]))

        # Give target model the same observation
        self.target_model(Variable(B['obs'][:, eidx]))

        # compute Q2 based on next observation
        Q2 = self.target_model(Variable(B['obs2'][:, eidx]))

        #loss += self.forward(B['obs'][:, i], B['action'][:, i], B['reward'][:, i], B['obs2'][:, i], B['done'][:, i])

        # get stuff for final time frame
        actions = B['action'][:, eidx]
        rewards = B['reward'][:, eidx]
        dones   = B['done'][:, eidx]
